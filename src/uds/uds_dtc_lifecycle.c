    ...
    const LinkUdsDtcLifecycleDefinition *definition)
{
    return definition != NULL &&
           ...
           definition->failed_threshold > 0 &&
           definition->passed_threshold < 0 &&
    ...
    LinkUdsDtcLifecycleState *state)
{
    ...
    if (state->failed_this_cycle) {
        state->aging_counter = 0U;
        if (state->fault_detection_counter >= definition->failed_threshold) {
    ...
    if (!state->tested_this_cycle) return true;
