...
The genuine Mercedes me Adapter is handled as a distinct native transport rather than being made to impersonate an ELM327. Archived official Mercedes me Adapter 4.7.61 interoperability evidence, including generation-name rules and exact Bluetooth UUIDs, is preserved in [`docs/MERCEDES-ME-ADAPTER-INTEROP.md`](docs/MERCEDES-ME-ADAPTER-INTEROP.md). LINK recognises the documented `MB-xxxx` Bluetooth name, can establish the available RFCOMM/GATT byte channel and passively preserve incoming bytes, while deliberately suppressing ELM `ATI`/initialisation traffic. The archived native command builders now prove the classic 11-bit raw-CAN and ISO-TP command framing as well as the secure envelope. LINK therefore exposes a native Mercedes me diagnostic bridge that can compile the same transport-neutral request used by other adapters into the adapter's proved `I`/`i` command pair. Session establishment/authentication remains evidence-gated until the complete challenge ordering is reproduced and then checked against the physical adapter.
