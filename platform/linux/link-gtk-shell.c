...
static size_t shell_supported_pid_count(const LinkDiagnosticFlow *flow)
{
    size_t count = 0U;
    unsigned int raw;
...
static LinkDiagnosticCapabilityEvidence shell_capability_evidence(
    LinkGtkShell *shell)
{
    LinkDiagnosticCapabilityEvidence evidence = {0};
    const LinkDiagnosticFlow *flow;

    if (shell == NULL) return evidence;
    flow = &shell->flow;
    ...
    evidence.standard_obd_response =
        flow->supported_pid_responders.count != 0U ||
        shell_supported_pid_count(flow) != 0U ||
        flow->standard_vin_available ||
        flow->readiness_available ||
        flow->stored_dtcs.count != 0U ||
        flow->pending_dtcs.count != 0U ||
        flow->permanent_dtcs.count != 0U;

    if (shell->descriptor != NULL &&
        shell->descriptor->diagnostic_capability_evidence != NULL) {
        shell->descriptor->diagnostic_capability_evidence(
            &evidence, shell->descriptor->context);
    }
    return evidence;
}

static void append_standard_obd_workspace(
    LinkGtkShell *shell, GtkWidget *body)
{
    LinkDiagnosticCapabilityEvidence evidence;
    LinkDiagnosticTier tier;
    const LinkDiagnosticFlow *flow;
    ...
    flow = &shell->flow;
    evidence = shell_capability_evidence(shell);
    tier = link_diagnostic_capability_classify(&evidence);
    pid_count = shell_supported_pid_count(flow);
    (void)link_diagnostic_flow_freeze_frame_samples(flow, &freeze_count);

    capability = link_gtk_card_new(
        "DIAGNOSTIC GENERATION",
        "Evidence-based capability classification");
    link_gtk_card_append_status(
        capability,
        link_diagnostic_tier_name(tier),
        tier == LINK_DIAGNOSTIC_TIER_NONE ||
                tier == LINK_DIAGNOSTIC_TIER_UNKNOWN
            ? "state-warning" : "state-success");
    link_gtk_card_append_detail(
        capability, "Standard OBD response",
        evidence.standard_obd_response ? "Observed" : "Not yet observed");
    link_gtk_card_append_detail(
        capability, "Legacy diagnostic response",
        evidence.legacy_diagnostic_response ? "Observed" : "Not observed");
    link_gtk_card_append_note(
        capability, link_diagnostic_tier_summary(tier));
    gtk_box_append(GTK_BOX(body), capability);

    standard = link_gtk_card_new(
        "STANDARD OBD-II / EOBD",
        "LINK-owned generic diagnostic inventory");
    link_gtk_card_append_detail(
        standard, "VIN",
        flow->standard_vin_available && flow->standard_vin[0] != '\0'
            ? flow->standard_vin : "Unavailable / not yet read");
    (void)snprintf(
        value, sizeof(value), "%zu physical responder%s",
        flow->supported_pid_responders.count,
        flow->supported_pid_responders.count == 1U ? "" : "s");
    link_gtk_card_append_detail(standard, "Responders", value);
    (void)snprintf(value, sizeof(value), "%zu advertised PID%s",
                   pid_count, pid_count == 1U ? "" : "s");
    link_gtk_card_append_detail(standard, "Mode 01 live data", value);

    if (flow->readiness_available) {
        (void)snprintf(
            value, sizeof(value), "%s · %u confirmed DTC%s · %s ignition",
            flow->readiness.mil_on ? "MIL on" : "MIL off",
            (unsigned int)flow->readiness.confirmed_dtc_count,
            flow->readiness.confirmed_dtc_count == 1U ? "" : "s",
            flow->readiness.compression_ignition ? "compression" : "spark");
    } else {
        (void)snprintf(value, sizeof(value), "%s",
                       flow->readiness_attempted
                           ? "Unavailable / unsupported" : "Not yet collected");
    }
    link_gtk_card_append_detail(standard, "Readiness", value);

    (void)snprintf(
        value, sizeof(value), "%zu stored · %zu pending · %zu permanent",
        flow->stored_dtcs.count,
        flow->pending_dtcs.count,
        flow->permanent_dtcs.count);
    link_gtk_card_append_detail(standard, "Fault memory", value);
    (void)snprintf(
        value, sizeof(value), "%zu captured sample%s",
        freeze_count, freeze_count == 1U ? "" : "s");
    link_gtk_card_append_detail(standard, "Mode 02 freeze frame", value);
    link_gtk_card_append_note(
        standard,
        "Detailed faults and live values remain available in the shared Faults, Live Data and Table workspaces.");
    gtk_box_append(GTK_BOX(body), standard);

    coverage = link_gtk_card_new(
        "COMMON COVERAGE",
        "Portable LINK standards surface");
    link_gtk_card_append_detail(
        coverage, "Mode 01", "Supported-PID discovery, readiness and current data");
    link_gtk_card_append_detail(
        coverage, "Mode 02", "Bounded freeze-frame context");
    link_gtk_card_append_detail(
        coverage, "Modes 03 / 07 / 0A", "Stored, pending and permanent DTC inventory");
    link_gtk_card_append_detail(
        coverage, "Mode 09", "Vehicle information including standard VIN where available");
    link_gtk_card_append_detail(
        coverage, "OBDonUDS", "SAE J1979-2 foundation through LINK UDS/ISO-TP");
    link_gtk_card_append_note(
        coverage,
        "Legacy and transitional details come from the manufacturer layer while LINK owns this common presentation.");
    gtk_box_append(GTK_BOX(body), coverage);
}
...
static void render_current_section_now(LinkGtkShell *shell)
{
    ...
    do {
         ...
         */
        {
            ...
            if (shell->current_section == LINK_WORKSPACE_OBD) {
                append_standard_obd_workspace(shell, body);
            } else if (shell->descriptor->render_section != NULL) {
