...
bool uds_c092_fdcan_tx_complete(void *context) {
    ...
    /* No completion event has been observed.  Idle is not the same thing as
     * a newly completed frame; returning true here caused repeated spurious
     * endpoint completion notifications in the original KEIL integration. */
    return false;
}
