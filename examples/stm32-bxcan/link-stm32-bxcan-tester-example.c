...
/**
 * @file link-stm32-bxcan-tester-example.c
 * @brief bxCAN UDS tester/client example; this file never acts as an ECU.
 *
 * Use link-stm32f103-uds-ecu.c plus issue-32/Src-main.c for an STM32F103
 * ECU/server that receives tester requests and sends UDS responses.
 */
...
static LinkStm32BxCanTesterExampleState example_state =
    LINK_STM32_BXCAN_TESTER_EXAMPLE_IDLE;
...
void link_stm32_bxcan_tester_example_default_tester_config(
...
bool link_stm32_bxcan_tester_example_init_tester(
    ...
    const LinkStm32BxCanTesterConfig *tester_config)
{
    ...
    example_nrc = 0U;
    example_state = LINK_STM32_BXCAN_TESTER_EXAMPLE_IDLE;
    ...
    if (!link_stm32_can_init(&example_can, &ops) ||
        ...
        example_state = LINK_STM32_BXCAN_TESTER_EXAMPLE_FAILED;
        return false;
    }
    ...
    if (!tester_config->read_vin_on_init) {
        example_state = LINK_STM32_BXCAN_TESTER_EXAMPLE_READY;
        return true;
    }
    ...
    example_state = LINK_STM32_BXCAN_TESTER_EXAMPLE_READING_VIN;
    return true;
}

bool link_stm32_bxcan_tester_example_init(CAN_HandleTypeDef *hcan)
{
    LinkStm32BxCanTesterConfig config =
        LINK_STM32_BXCAN_TESTER_CONFIG_INIT;

    return link_stm32_bxcan_tester_example_init_tester(hcan, &config);
}

bool link_stm32_bxcan_tester_example_start_vin(void)
{
    if (example_state != LINK_STM32_BXCAN_TESTER_EXAMPLE_READY &&
        example_state != LINK_STM32_BXCAN_TESTER_EXAMPLE_VIN_READY &&
        example_state != LINK_STM32_BXCAN_TESTER_EXAMPLE_NEGATIVE_RESPONSE) {
        return false;
    }
...
void link_stm32_bxcan_tester_example_process(void)
{
    LinkStm32UdsResult result;
    const LinkUdsResponse *response;

    if (example_state != LINK_STM32_BXCAN_TESTER_EXAMPLE_READING_VIN) {
        return;
    }
...
void link_stm32_bxcan_tester_example_rx_fifo0_irq(CAN_HandleTypeDef *hcan)
{
...
void link_stm32_bxcan_tester_example_tx_complete_irq(
    CAN_HandleTypeDef *hcan,
    uint32_t mailbox)
{
    if (hcan != NULL && hcan == example_hal.hcan) {
...
LinkStm32BxCanTesterExampleState link_stm32_bxcan_tester_example_state(void)
{
    return example_state;
}

const char *link_stm32_bxcan_tester_example_vin(void)
{
    return example_state == LINK_STM32_BXCAN_TESTER_EXAMPLE_VIN_READY
        ? example_vin : NULL;
}

uint8_t link_stm32_bxcan_tester_example_negative_response_code(void)
{
    return example_nrc;
}

uint32_t link_stm32_bxcan_tester_example_dropped_frames(void)
{
