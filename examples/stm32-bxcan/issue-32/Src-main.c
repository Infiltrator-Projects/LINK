...
{
    if (hcan == &hcan1) {
        link_stm32_bxcan_hal_tx_complete_irq(&uds_hal, CAN_TX_MAILBOX0);
    }
}
...
{
    if (hcan == &hcan1) {
        link_stm32_bxcan_hal_tx_complete_irq(&uds_hal, CAN_TX_MAILBOX1);
    }
}
...
{
    if (hcan == &hcan1) {
        link_stm32_bxcan_hal_tx_complete_irq(&uds_hal, CAN_TX_MAILBOX2);
    }
}
...
{
    if (hcan == &hcan1) {
        link_stm32_bxcan_hal_tx_abort_irq(&uds_hal, CAN_TX_MAILBOX0);
    }
}
...
{
    if (hcan == &hcan1) {
        link_stm32_bxcan_hal_tx_abort_irq(&uds_hal, CAN_TX_MAILBOX1);
    }
}
...
{
    if (hcan == &hcan1) {
        link_stm32_bxcan_hal_tx_abort_irq(&uds_hal, CAN_TX_MAILBOX2);
    }
}

void HAL_CAN_ErrorCallback(CAN_HandleTypeDef *hcan)
{
    if (hcan == &hcan1) link_stm32_bxcan_hal_error_irq(&uds_hal);
}
